...
public class CharacterInventory : NetworkBehaviour
{
	...
	/*public void GetItemFromServer(string itemid,int num,int numtag,int shortcut){

	}

	[TargetRpc]
	public void TargetGetItemFromServer (NetworkConnection target, string itemid,int num,int numtag,int shortcut)
	{
		ItemData item = UnitZ.itemManager.GetItemDataByID(itemid);
		if(item){
			AddItemByItemData(item,num,numtag,shortcut);
		}
	} */
	...
	public void AttachFPSItemView (FPSItemEquipment item)
	{
		...
		if (item != null && FPSItemView != null) {
			...
			//Debug.Log(" 1 Attach FPS "+newitem+"  "+newitem.transform.parent);
	...
	public void AttachFPSItemViewAndCollector (FPSItemEquipment item, ItemCollector itemcollector)
	{
		...
		if (item != null && FPSItemView != null) {
			...
			//Debug.Log(" 2 Attach FPS "+newitem+"  "+newitem.transform.parent);
	...
	void Update ()
	{
		...
		if (isLocalPlayer && IsReady) {
			// send all inventory info everyhing time delayed by UpdateDelay parameter
			if (Time.time >= updateTimeTmp + UpdateDelay) {
				string stickertext = GenStickerTextData ();
				CmdUpdateOtherInventory (stickertext, UpdateCount);
		...
		if (!IsReady) {
			if (StickerTextData != GenStickerTextData ()) {
				ReSetupStickers (StickerTextData);
			}
		}
	}

	[Command(channel = 1)]
	public void CmdUpdateOtherInventory (string text, int updatecout)
	{
		if (text != StickerTextData) {
			UpdateCount = updatecout;
	...
	{
		StickerTextData = text;
		CmdUpdateOtherInventory (StickerTextData, UpdateCount);
		IsReady = true;
	}
	...
	public void OnInventoryChanged (string text)
	{
		StickerTextData = text;
		if (!isLocalPlayer)
			ReSetupStickers (text);
	}

	public void ReSetupStickers (string text)
	{
		if (text == "")
			return;
